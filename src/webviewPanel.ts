...
// Singleton Webview that displays:
// 1. 5h window summary per quota group (AGP)
// 2. Archived windows history (AGP)
// 3. Rolling 7-day usage statistics table (from Context Window Monitor)
// Data comes from UsageStore + WindowTracker (in-memory) — no RPC calls on open.
...
import { QUOTA_GROUP_LABELS, ALL_QUOTA_GROUPS, QuotaGroup } from './modelGroups';

// ─── Constants ────────────────────────────────────────────────────────────────

const VIEW_TYPE = 'agpUsageReport';
const WINDOW_DAYS = 7;
...
export class UsageReportPanel {
  ...
  private constructor(panel: vscode.WebviewPanel, store: UsageStore, windowTracker: WindowTracker | null) {
    this.panel = panel;
    this.store = store;
  ...
  static createOrShow(context: vscode.ExtensionContext, store: UsageStore, windowTracker?: WindowTracker): void {
    ...
    const panel = vscode.window.createWebviewPanel(
      VIEW_TYPE,
      '📊 AG Pulse Report / 用量报告',
      vscode.ViewColumn.One,
      { enableScripts: true, retainContextWhenHidden: true }
    );
    UsageReportPanel.instance = new UsageReportPanel(panel, store, windowTracker || null);
...
<body>
  <h2>📊 AG Pulse Report / 用量报告</h2>

  <!-- AGP: 5h Window Summary -->
  ${this.buildWindowHtml()}
  ...
  private buildWindowHtml(): string {
    ...
    const archived = this.windowTracker.getArchivedWindows();

    if (summary.groups.length === 0 && archived.length === 0) {
      return '';
    }

    let html = `<div class="section-title">⏱️ 5h Window / 5h 窗口</div>`;
    ...
    if (summary.groups.length > 0) {
      ...
      for (const g of summary.groups) {
        ...
        html += `
        <div class="card">
          <div class="label">${this.escapeHtml(g.label)}</div>
          <div class="value">⏱️ ${remaining}</div>
          <div class="sub">
            📊 ${fmtTokens(g.totalTokens)} tok | 💬 ${g.sendCount} sends | 💰 ${formatCost(g.totalCost)}
            ${g.retryCount > 0 ? `| 🔄 ${g.retryCount} retries` : ''}
          </div>
        </div>`;
      }
      html += `</div>`;
    ...
    if (archived.length > 0) {
      html += `<div class="section-title">📦 Archived Windows / 已归档窗口</div>`;
      html += `<table>
        <tr>
          <th>Group / 组</th>
      ...
      const recent = archived.slice().reverse().slice(0, 20);
      for (const w of recent) {
        const label = QUOTA_GROUP_LABELS[w.quotaGroup] || w.quotaGroup;
        const totalTokens = w.totalInputTokens + w.totalOutputTokens;
        html += `<tr>
          <td>${this.escapeHtml(label)}</td>
          <td class="num">${this.formatTimestamp(w.startTime)}</td>
          <td class="num">${this.formatTimestamp(w.endTime)}</td>
          <td class="num">${fmtTokens(totalTokens)}</td>
          <td class="num">${w.sendCount}</td>
          <td class="num">${w.retryCount}</td>
          <td class="num">${formatCost(w.totalCost)}</td>
        </tr>`;
      }
      html += `</table>`;
    }
  ...
  private formatTimestamp(ms: number): string {
    const d = new Date(ms);
    return `${fmtDateLocal(d)} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
  }
