...
export function activate(context: vscode.ExtensionContext): void {
    ...
    disposed = false;
    outputChannel = vscode.window.createOutputChannel('AG Pulse');
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agp.showDetails', () => {
            statusBar.showDetailsPanel(currentUsage, allTrajectoryUsages);
        }),
        vscode.commands.registerCommand('agp.refresh', () => {
        ...
        vscode.commands.registerCommand('agp.showReport', () => {
            UsageReportPanel.createOrShow(extensionContext, usageStore, windowTracker);
        }),
        statusBar,
        outputChannel
    );
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('agp.pollingInterval')) {
                const newConfig = vscode.workspace.getConfiguration('agp');
            ...
            if (e.affectsConfiguration('agp.windowDurationHours')) {
                const newConfig = vscode.workspace.getConfiguration('agp');
                const newDuration = Math.max(1, newConfig.get<number>('windowDurationHours', 5));
                windowTracker = new WindowTracker(extensionContext.globalState, newDuration);
                statusBar.setWindowTracker(windowTracker);
                log(`Window duration reconfigured: ${newDuration}h`);
            }
        })
    );
...
async function pollContextUsage(): Promise<void> {
    ...
    try {
        ...
        if (trajectories.length === 0) {
            ...
            const config0 = vscode.workspace.getConfiguration('agp');
