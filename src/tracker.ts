...
export interface TokenUsageResult {
    ...
    /** AGP: Number of user input steps (sends/messages) */
    sendCount: number;
    /** AGP: Number of planner response steps */
    responseCount: number;
    /** AGP: Total retry count across all checkpoints */
    retryCount: number;
    /** AGP: Collected retry token usage from checkpoint retryInfos */
    retryInfos: Array<{ inputTokens: number; outputTokens: number }>;
}
...
export function processSteps(steps: Array<Record<string, unknown>>): TokenUsageResult {
    ...
    let responseCount = 0;
    let retryCount = 0;
    const collectedRetryInfos: Array<{ inputTokens: number; outputTokens: number }> = [];
    ...
    for (let globalStepIdx = 0; globalStepIdx < steps.length; globalStepIdx++) {
        ...
        if (stepType === 'CORTEX_STEP_TYPE_PLANNER_RESPONSE') {
            responseCount++;  // AGP: count responses
            ...
            if (mu) {
                ...
                if (retryInfos && retryInfos.length > 0) {
                    ...
                    for (const retry of retryInfos) {
                        ...
                        if (usage) {
                            const rIn = parseInt(String(usage.inputTokens || '0'), 10);
                            const rOut = parseInt(String(usage.outputTokens || '0'), 10);
                            retryInputTokens += rIn;
                            retryOutputTokens += rOut;
                            // AGP: Collect individual retry token data
                            collectedRetryInfos.push({ inputTokens: rIn, outputTokens: rOut });
                        }
                    }
                    ...
                    console.log(
                        `[AGP] Checkpoint retryInfos: ${retryInfos.length} retries, ` +
    ...
    if (lastModelUsage && lastModelUsage.inputTokens > 0) {
        return {
            ...
            responseCount,
            retryCount,
            retryInfos: collectedRetryInfos,
        };
    }
