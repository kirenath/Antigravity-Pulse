...
import { WindowTracker, WindowState } from './pulse';
import { QuotaGroup, QUOTA_GROUP_LABELS, ALL_QUOTA_GROUPS } from './modelGroups';
...
export class StatusBarManager {
    ...
    constructor(windowTracker?: WindowTracker) {
        this.windowTracker = windowTracker || null;
        this.statusBarItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Right,
            100
        );
        this.statusBarItem.command = 'agp.showReport';
        this.statusBarItem.name = 'AG Pulse / 脉冲监控';
        this.showInitializing();
        this.statusBarItem.show();
    }

    /** Update the WindowTracker reference (e.g. after reconfiguration). */
    setWindowTracker(tracker: WindowTracker): void {
    ...
    showInitializing(): void {
        this.statusBarItem.text = '$(sync~spin) AG Pulse...';
        this.statusBarItem.tooltip = 'AG Pulse: Initializing / 初始化中...';
    ...
    showDisconnected(message: string): void {
        this.statusBarItem.text = '$(debug-disconnect) AGP: N/A';
        this.statusBarItem.tooltip = `AG Pulse: ${message}`;
        this.statusBarItem.backgroundColor = undefined;
    }
    ...
    showNoConversation(limitStr: string = '1000k'): void {
        const windowLine = this.buildWindowStatusLine();
        const windowSuffix = windowLine ? ` | ${windowLine}` : '';
        this.statusBarItem.text = `$(comment-discussion) 0k/${limitStr}, 0.0%${windowSuffix}`;
        const md = new vscode.MarkdownString(
            `AG Pulse: No active conversation / 无活跃会话  \n${this.buildWindowTooltipLines().join('  \n')}\nClick to view details / 点击查看详情`,
            false
        );
    ...
    showIdle(limitStr: string = '1000k'): void {
        const windowLine = this.buildWindowStatusLine();
        const windowSuffix = windowLine ? ` | ${windowLine}` : '';
        this.statusBarItem.text = `$(clock) 0k/${limitStr}, 0.0%${windowSuffix}`;
        const md = new vscode.MarkdownString(
            `AG Pulse: Idle / 空闲  \nNew or ended conversation  \n${this.buildWindowTooltipLines().join('  \n')}\nClick to view details / 点击查看详情`,
            false
        );
        md.supportThemeIcons = true;
        this.statusBarItem.tooltip = md;
    ...
    update(usage: ContextUsage): void {
        ...
        // AGP: Append 5h window summary line to status bar text
        const windowLine = this.buildWindowStatusLine();
        const windowSuffix = windowLine ? ` | ${windowLine}` : '';

        this.statusBarItem.text = `${icon} ${usedStr}/${limitStr}, ${displayPercent}%${compressIcon}${gapsIndicator} | ${costStr}${windowSuffix}`;
        ...
        // AGP: 5h window information in tooltip
        const windowTooltipLines = this.buildWindowTooltipLines();
        if (windowTooltipLines.length > 0) {
            lines.push(`——————————`);
            for (const wl of windowTooltipLines) { lines.push(wl); }
        }
    ...
    async showDetailsPanel(
        ...
        await vscode.window.showQuickPick(items, {
            title: '📊 AG Pulse Context Monitor / 上下文窗口使用情况',
            placeHolder: 'View context details for all sessions / 查看各会话的上下文使用详情',
            canPickMany: false
        });
    }

    // ── AGP: 5h Window Helpers ────────────────────────────────────────────

    /**
     * Build a compact status bar string for the most active 5h window.
     * Format: ⏱️ 2h13m | 📊 45.2K | 💬 23 | 💰 $1.82
     */
    private buildWindowStatusLine(): string {
        if (!this.windowTracker) { return ''; }

        const summary = this.windowTracker.getSummary();
        if (summary.groups.length === 0) { return ''; }

        // Show the group with the highest cost (most active)
        const primary = summary.groups.reduce((a, b) => a.totalCost >= b.totalCost ? a : b);
        const remaining = this.formatRemainingTime(primary.remainingMs);
        const tokens = formatTokenCount(primary.totalTokens);

        return `⏱️${remaining} 📊${tokens} 💬${primary.sendCount}`;
    }

    /**
     * Build detailed tooltip lines for all 5h windows.
     */
    private buildWindowTooltipLines(): string[] {
        if (!this.windowTracker) { return []; }
        ...
        const lines: string[] = [
            `⏱️ 5h Window Status / 5h 窗口状态`,
        ];

        for (const g of summary.groups) {
            const remaining = this.formatRemainingTime(g.remainingMs);
            const tokens = formatTokenCount(g.totalTokens);
            const cost = formatCost(g.totalCost);
            const retryStr = g.retryCount > 0 ? ` | 🔄 ${g.retryCount} retries` : '';
            lines.push(
                `  ${g.label}: ⏱️ ${remaining} | 📊 ${tokens} tok | 💬 ${g.sendCount} sends | 💰 ${cost}${retryStr}`
            );
        }
